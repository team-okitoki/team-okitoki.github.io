...
이번 블로그에서는 컴퓨트, 스토리지 구성, DB 설치, RAC 구성까지 원클릭 프로비저닝을 통해서 자동으로 구성을 해 주는 Virtual Machine, Bare Metal 기반의 데이터베이스 서비스인 Base Database Cloud Service 를 빠르게 Provisioning 하고 사용하는 방법에 대해서 알아보도록 하겠습니다.
(※ 이하 Base Database Cloud Service 를 편의상 DBCS 로 칭하겠습니다.)
...
DBCS 생성은 매우 간단한 절차에 의해 자동으로 Oracle Database 서비스를 구성할 수 있도록 되어 있습니다.
...
- DBCS 의 목록 화면이 나오면 Oracle Database 서비스 자원이 위치할 Compartment (구획) 이 잘 선택되어 있는지 확인 후 "Create DB System" 버튼을 클릭합니다.
...
- Create DB System 화면에서는 DBCS 생성을 위한 다음과 같은 사항들을 입력하거나 선택 후 아래로 스크롤 다운 합니다.
    ...
    - Select an availability domain : AD-1 (AD domain 이 여러개 있는 Region 에서는 AD 를 선택할 수 있음)
...
- 기본 선택되어 있는 VM.Standard.E4.Flex 사용하도록 선택하고, 하단의 "Change Storage" 버튼을 클릭하면 Storage 의 성능을 높일 수 있는 화면이 나타납니다. Shape 선택과 Storage 를 선택 후 아래로 스크롤 다운합니다. (※ 기본 선택된 Shape 과 Storage 사용)  
    ...
    - Storage Management Software 선택
        - Oracle Grid Infrastructure : Oracle 의 RAC 환경 구성을 위한 Grid 지원 Storage Software 입니다. RAC 구성 시에는 반드시 선택해야 합니다. (기본 선택 사용)
    ...
    - Storage performance 선택
        - Balanced : 기본 성능 수준의 스토리지로 대부분의 워크로드에 대해 성능과 비용 절감 간의 적절한 균형을 제공합니다
        - Higher performance : 대용량 데이터베이스를 포함하여 최상의 성능을 요구하는 가장 IO를 많이 요구하는 워크로드에 권장되는 선택입니다. (기본 선택 사용)

    ![Windows Preparation](/assets/img/dataplatform/2022/dbcs/quickstart/07.oci-dbcs-create-db-system-input-2-storage-2.png)

    - 사용할 Storage 용량도 지정할 수 있습니다. Available Data Storage 에 사이즈를 지정하면 DBCS 가 사용하는 스토리지 용량은 Available Data Storage 사이즈의 2배입니다. Available Data Storage 영역과 동일한 사이즈로 Recovery 영역을 자동으로 지정되게 됩니다.
...
- 다음 단계에서는 Database 정보를 입력하는 중요한 단계입니다. 아래 화면과 같이 입력 후 스크롤 다운합니다. 
