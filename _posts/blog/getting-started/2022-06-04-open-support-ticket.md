...
티켓 생성의 경우 OCI Tenancy를 생성할 때 만든 관리자 계정(Cloud Account Admin 권한을 갖는 사용자)은 기본적으로 생성 가능하지만, 다른 사용자가 티켓을 생성하기 위해서는 오라클 서포트 계정을 프로비저닝하고 Cloud Account Admin으로부터 승인을 받아야 합니다. OCI 사용자 계정을 Oracle 지원 계정으로 사용하기 위한 절차를 수행해야 합니다. 해당 절차는 다음 포스팅을 참고합니다.
...
기술 지원을 위한 SR 티켓을 생성하기 위해서는 다음 항목에 내용을 제공해야 합니다.
